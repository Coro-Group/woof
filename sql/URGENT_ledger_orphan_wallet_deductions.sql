-- URGENT: Include wallet deductions on voided/consolidated invoices in ledger
-- Paste into Supabase SQL editor (project wineliuwejkxwsdbrthb).
-- Source: supabase/migrations/20260908192000_ledger_orphan_wallet_deductions.sql
--
...
-- =============================================================================
-- 0) Confirm is_ledger_wallet_event excludes any non-null invoice_id
-- =============================================================================
SELECT
  public.is_ledger_wallet_event(NULL::uuid, 'top_up'::public.transaction_type) AS standalone_topup,
  public.is_ledger_wallet_event(
    '00000000-0000-0000-0000-000000000001'::uuid,
    'deduction'::public.transaction_type
  ) AS invoice_linked_deduction;
-- Expect: true, false

-- =============================================================================
-- 1) BEFORE: the 4 orphan wallet deductions (Roshni) — sum should be -77.00
-- =============================================================================
SELECT
  wt.id,
  i.invoice_number,
  ...
  public.is_ledger_wallet_event(wt.invoice_id, wt.transaction_type) AS currently_in_wallet_events
FROM public.wallet_transactions wt
JOIN public.invoices i ON i.id = wt.invoice_id
WHERE wt.owner_id = '8a4b428d-b0c2-4b64-88d4-41c447d3f9e9'::uuid
  AND wt.transaction_type = 'deduction'::public.transaction_type
  AND NOT public.is_soa_invoice_status(i.status::text)
ORDER BY wt.created_at;

SELECT
  ROUND(COALESCE(SUM(wt.amount), 0), 2) AS orphan_deduction_sum
FROM public.wallet_transactions wt
JOIN public.invoices i ON i.id = wt.invoice_id
WHERE wt.owner_id = '8a4b428d-b0c2-4b64-88d4-41c447d3f9e9'::uuid
  AND wt.transaction_type = 'deduction'::public.transaction_type
  AND NOT public.is_soa_invoice_status(i.status::text);
-- Expect orphan_deduction_sum = -77.00

SELECT balance_after AS running_ledger_before
...
LIMIT 1;
-- Expect ~895.75 before this fix (after payment_events orphan fix)
...
-- =============================================================================
-- 3) AFTER: Roshni last Running Ledger must be 818.75
-- =============================================================================
SELECT
  row_id,
  created_at,
...
SELECT balance_after AS running_ledger_after
FROM public.get_ledger_statement(
  '8a4b428d-b0c2-4b64-88d4-41c447d3f9e9'::uuid,
  '2020-01-01 00:00:00+04'::timestamptz,
  now()
...
-- Spot-check: owners with wallet deductions on voided/consolidated invoices
WITH candidates AS (
  SELECT DISTINCT wt.owner_id
  FROM public.wallet_transactions wt
  JOIN public.invoices i ON i.id = wt.invoice_id
  WHERE wt.transaction_type = 'deduction'::public.transaction_type
    AND NOT public.is_soa_invoice_status(i.status::text)
    AND wt.owner_id <> '8a4b428d-b0c2-4b64-88d4-41c447d3f9e9'::uuid
  LIMIT 3
)
SELECT
  o.id AS owner_id,
  ...
  (
    SELECT ROUND(COALESCE(SUM(wt.amount), 0), 2)
    FROM public.wallet_transactions wt
    JOIN public.invoices i ON i.id = wt.invoice_id
    WHERE wt.owner_id = o.id
      AND wt.transaction_type = 'deduction'::public.transaction_type
      AND NOT public.is_soa_invoice_status(i.status::text)
  ) AS orphan_deduction_sum
FROM candidates c
