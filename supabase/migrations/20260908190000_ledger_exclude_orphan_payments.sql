-- Exclude invoice_payments whose parent invoice is not SOA-eligible.
-- Root cause: invoice_events filter with is_soa_invoice_status, but payment_events
-- did not — so card credits remained after voided/consolidated/cancelled/draft
-- invoices dropped their debit rows (orphan Running Ledger inflation).
...
-- Verification:
-- SELECT i.invoice_number, i.status, ROUND(SUM(ip.amount), 2) AS orphan_credits
-- FROM invoice_payments ip
-- JOIN invoices i ON i.id = ip.invoice_id
-- WHERE ip.owner_id = '8a4b428d-b0c2-4b64-88d4-41c447d3f9e9'
--   AND NOT public.is_soa_invoice_status(i.status::text)
--   AND ip.payment_method <> 'wallet'
...
-- -- expect 818.75 for Roshni (wallet − open invoices)
