-- Recompute invoices.amount_paid from SUM(invoice_payments) on DELETE/UPDATE
-- as well as INSERT. Manual cleanup of duplicate payment rows previously left
-- amount_paid inflated because trg_update_invoice_status_on_payment was INSERT-only.
...
BEGIN
  ...
  -- Never resurrect voided / consolidated invoices via payment row churn.
...
BEGIN
  ...
  -- If a payment row moves between invoices, refresh the previous invoice too.
...
-- Replace INSERT-only trigger with INSERT OR UPDATE OR DELETE.
...
-- -- expect AFTER INSERT OR DELETE OR UPDATE
